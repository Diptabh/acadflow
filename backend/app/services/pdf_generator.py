...
def generate_ca2_topsheet(student_data: Dict[str, Any], eval_data: Dict[str, Any], faculty_data: Dict[str, Any], subject_data: Dict[str, Any]) -> str:
    """
    Builds MAKAUT-format PDF for CA2 Report Evaluation using ReportLab, injects signature PNG,
    saves to Supabase Storage, and returns file URL.
    """
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=30, leftMargin=30, topMargin=30, bottomMargin=30)
    ...
    title_style = ParagraphStyle(name='TitleStyle', parent=styles['Heading1'], alignment=1, spaceAfter=20)
    elements.append(Paragraph("MAKAUT CA2 Evaluation Topsheet", title_style))
    ...
    table_data = [["Criteria", "Marks Allotted", "Marks Awarded", "Remark"]]

    total_awarded = int(eval_data.get("marks", 0))
    total_allotted = 25
    remark = eval_data.get("ai_reasoning", "Evaluated manually")

    table_data.append(["Report Evaluation", str(total_allotted), str(total_awarded), remark])
    table_data.append(["Total", str(total_allotted), str(total_awarded), ""])

    table = Table(table_data, colWidths=[2.5*inch, 1.2*inch, 1.2*inch, 2*inch])
    table.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.grey),
        ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke),
        ('ALIGN', (0,0), (-1,-1), 'CENTER'),
    ...
    file_name = f"ca2_{student_data.get('university_roll')}_{subject_data.get('code')}_{uuid.uuid4()}.pdf"
